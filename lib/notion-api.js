...
import { toDashId, notionImage, idNoDash } from './util'
...
async function getFirstImageUrl(pageId, width = 800, attempt = 0) {
  try {
    const res = await notion.blocks.children.list({
    ...
    for (const b of res.results) {
      if (b.type === 'image') {
        ...
        if (url) return notionImage(url, b.id, width)
      }
    }
  } catch (e) {
    if (e?.code === 'rate_limited' && attempt < 2) {
      await new Promise((r) => setTimeout(r, 700 * (attempt + 1)))
      return getFirstImageUrl(pageId, width, attempt + 1)
    }
  }
  return null
}
...
export async function getCategories() {
  ...
  let cats = rows.map((p) => {
    ...
    return {
      ...
      cover: notionImage(coverUrl(p.cover), p.id, 800)
    }
  })
...
// Published posts of an inner database, newest first. No first-image fetching
// (cheap) — used for both the category grid and prev/next neighbors.
async function listPostsBasic(dbId) {
  ...
  let posts = rows.map((p) => {
    ...
    return {
      ...
      cover: notionImage(coverUrl(p.cover), p.id, 800),
      date: pickDate(pr, p.created_time), // post_date if present, else created
      created: p.created_time
    }
  })

  posts = posts.filter((p) => p.title && p.published)
  // Newest first by display date, tie-broken by created time.
  posts.sort((a, b) => {
...
async function getCategoryPosts(categoryPageId) {
  ...
  const posts = await listPostsBasic(dbBlock.id)
  ...
  await mapLimit(needThumb, 4, async (p) => {
    p.cover = await getFirstImageUrl(p.id, 800)
  })
...
export async function getCategory(categoryPageId) {
  ...
  return {
    ...
    cover: notionImage(coverUrl(page.cover), page.id, 1200),
    posts
  }
}
...
export async function getPage(pageId) {
  ...
  // Prev/next within the same category (the post's parent database).
  let prev = null
  let next = null
  if (page.parent?.type === 'database_id') {
    try {
      const siblings = await listPostsBasic(page.parent.database_id)
      const cur = idNoDash(pageId)
      const i = siblings.findIndex((s) => idNoDash(s.id) === cur)
      if (i !== -1) {
        // Newest-first list: newer is above (i-1), older is below (i+1).
        const newer = i > 0 ? siblings[i - 1] : null
        const older = i < siblings.length - 1 ? siblings[i + 1] : null
        if (older) prev = { id: idNoDash(older.id), title: older.title }
        if (newer) next = { id: idNoDash(newer.id), title: newer.title }
      }
    } catch (e) {
      // no neighbors on failure
    }
  }
  ...
  return {
    ...
    cover: notionImage(coverUrl(page.cover), page.id, 1400),
